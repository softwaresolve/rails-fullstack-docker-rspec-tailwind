...
```sh
git clone https://github.com/nickjj/docker-rails-example rails-template
cd rails-template
...
The app is named `rails-template` right now but chances are your app will be a different
name. Since the app is already created we'll need to do a find / replace on a
few variants of the string "rails-template" and update a few Docker related resources.
